...
import { Icon, Dialog, Button, Slider } from '@blueprintjs/core';
...
const CustomDownloadButton = observer(({ store }) => {
    ...
    const handleDownload = async () => {
        try {
            if (fileType === 'PNG') {
                // Menggunakan store.saveAsImage() sesuai dokumentasi
                await store.saveAsImage({
                    width: Math.round(width * quality),
                    height: Math.round(height * quality),
                    mimeType: 'image/png'
                });
                ...
                // Menggunakan store.saveAsPDF() sesuai dokumentasi
                await store.saveAsPDF({
                    width: Math.round(width * quality),
                    height: Math.round(height * quality)
                });
            } else if (fileType === 'SVG') {
                // Menggunakan store.saveAsSVG() sesuai dokumentasi (experimental)
                await store.saveAsSVG({
                    width: width,
                    height: height
                });
            } else if (fileType === 'HTML') {
                // Menggunakan store.saveAsHTML() sesuai dokumentasi (experimental)
                await store.saveAsHTML({
                    width: width,
                    height: height
                });
            } else if (fileType === 'GIF') {
                // Menggunakan store.saveAsGIF() sesuai dokumentasi
                ...
                // Menggunakan store.toJSON() sesuai dokumentasi
